...
- [Building Bitcoin](#building-halobitcoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-halobitcoin-host/README.md).
...
Clone the git repositories for halobitcoin and gitian and then checkout the halobitcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/halobitcoin/halobitcoin
cd halobitcoin
...
In [doc/release-process.md](release-process.md) in the halobitcoin repository under 'Fetch and build inputs'.
...
gitian builds' in [doc/release-process.md](release-process.md) in the halobitcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/halobitcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/halobitcoin/halobitcoin
...
For example:
```bash
URL=https://github.com/laanwj/halobitcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit halobitcoin=${COMMIT} --url halobitcoin=${URL} ../halobitcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit halobitcoin=${COMMIT} --url halobitcoin=${URL} ../halobitcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit halobitcoin=${COMMIT} --url halobitcoin=${URL} ../halobitcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/halobitcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/halobitcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/halobitcoin-build.assert
```
...
[halobitcoin/gitian.sigs](https://github.com/halobitcoin/gitian.sigs/) repository, or if that's not possible create a pull
